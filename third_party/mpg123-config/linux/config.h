/* src/config.h — DESKTOP LINUX (glibc), libmpg123 subset only.
 *
 * Provenance: a VERBATIM copy of ../android/config.h (autotools-generated for
 * mpg123 1.32.10), not a re-run of ./configure — initialize_files.py extracts
 * only src/{libmpg123,compat,common,include}, so the tarball's configure
 * script is not part of that subset.
 *
 * Copying it is sound because only libmpg123 is compiled here: no CLI tools,
 * no output modules, no networking. The decoder subset consults exactly 19
 * HAVE_* macros, and Android is Linux with the same word size and endianness,
 * so every one carries over. The two that looked like candidate deltas were
 * checked against this machine's glibc and are correct as-is:
 *
 *   HAVE_O_LARGEFILE  stays undef — glibc gates O_LARGEFILE behind
 *                     _GNU_SOURCE, which this build does not define, so a
 *                     probe without it fails exactly as it did on Android.
 *   HAVE_LIMITS_H     stays ABSENT — mpg123's configure.ac never tests for
 *                     <limits.h>, so autoheader emits no entry for it at all.
 *                     Adding one would invent a macro upstream does not have.
 *
 *   HAVE_SETMODE / HAVE__SETMODE are Windows-only and undef on both.
 *
 * To regenerate properly instead: fetch a full mpg123 tarball and run
 * `./configure --with-cpu=generic --disable-modules`, then copy src/config.h
 * here. Version pin and update steps live in initialize_files.py.
 */
